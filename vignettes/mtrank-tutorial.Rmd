...
author: "Thodoris Evrenoglou, Guido Schwarzer"
output:
...
R function forest.mtrank() can be used to create a forest plot of ability estimates.

By default, log-ability estimates are shown in the forest plot.

```{r, eval = FALSE}
forest(fit, spacing = 1.5)
```
...
```{r, eval = FALSE}
forest(fit, spacing = 1.5, backtransf = TRUE)
```
...
```{r}
# Get probability that bupropion is better than trazodone
...
# Get probability that bupropion is worse than trazodone
paired_pref(fit, treat1 = "bupropion", treat2 = "trazodone",
  type = "worse")
...
paired_pref(fit, treat1 = "bupr",
  treat2 = c("fluo", "paro", "sert", "traz", "venl"),
  type = "better")
```
