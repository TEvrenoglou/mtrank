...
In the next step, we set the treatment-choice criterion and get treatment preferences from NMA estimates
...
As argument 'swd = 1.25' the ROE is defined as [1 / swd, swd] = [0.8, 1.25].
...
- x: an mtrank object,
- swd.vec: a numeric vector of SWD values to be used for the sensitivity analysis,
- swd.ref: a numeric SWD value to be used as the reference for sorting treatments in the final graph. This value should be included in swd.vec,
...
- ...: additional arguments passed to mtrank or ggplot.
...
linegraph(fit, swd.vec = swd.vec, swd.ref = swd.ref, k = 6)
