...
#' @param l.roe A numeric value specifying the lower limit of the range of equivalence
#' @param u.roe A numeric value specifying the upper limit of the range of equivalence
#' @param small.values A character string specifying whether small 
#' treatment effects indicate a beneficial (\code{"desirable"}) or
#' harmful (\code{"undesirable"}) effect.
#' @param relax.roe A logical optional argument. If TRUE it 'relaxes' the TCC 
...
#' 
#' The treatment choice criterion is defined primarily based on the arguments \code{l.roe} and \code{u.roe} 
#' which are mandatory and declare the lower and upper limits of the range of equivalence (ROE). Then, the 
#' the argument \code{relax.roe} is also used to declare whether the statistical signficance of the treatment effect
#' should also be considered when indicating a treatment preference or a tie. If set as FALSE then the treatment choice criterion
#' is equivalent to the one described by Evrenoglou et al., 2024. In this case the study-specific treatment effect
#' need to be both statistically and clinically significant to indicate a treatment preference. If set to TRUE
#' the criterion is relaxed and the study-specific treatment effects need to be only clinically signficant to
...
TCC <- function(treat,
                event,n,mean,sd,
                data=NULL,studlab,
                l.roe,u.roe,small.values="desirable",relax.roe=F,
                sm,
                ...){
  ...
  if(missing(l.roe)){
    stop("Argument 'l.roe' mandatory. Please specify the lower bound of the range of equivalence.")
  }
  
  if(missing(u.roe)){
    stop("Argument 'u.roe' mandatory. Please specify the upper bound of the range of equivalence.")
  }
  ...
  ## the argument relax.roe defines how the ROE should be treated. The default is FALSE. If TRUE, it relaxes the TTC shown in the main manuscript.
  ...
  for(i in 1:length(u)){
    ...
    r1[[i]]$L <- r1[[i]]$TE - 1.96*r1[[i]]$seTE
    
    r1[[i]]$U <- r1[[i]]$TE + 1.96*r1[[i]]$seTE
    ...
    if(small.values=="undesirable"){ ## since 'small.values' are undesirable the larger the treatment effect the more likely that for the comparison t1 vs t2, t1 is preferred.
      ...
      for (j in 1:nrow(r1[[i]])){
        ...
        ## for the pairwise comparison between treatments t1 and t2 the treatment 1 is in the second position and the treatment 2 in the first position (i.e. t2>t1)
        ## here 'no_effect2' can be either the null value (if relax.roe=FALSE (default)) or it can be the upper bound of ROE if relax.roe=T.
        ...
        ## for the pairwise comparison between treatments t1 and t2 the treatment 1 is in the first position and the treatment 2 in the second position (i.e. t1>t2)
        ## here 'no_effect1' can be either the null value (if relax.roe=FALSE (default)) or it can be the lower bound of ROE if relax.roe=T.
        ## the relaxed criterion allows for more "wins" to be identified from the TTC as in this case the statistical significance of the effect does not alter the preference
      ...
      for (j in 1:nrow(r1[[i]])){
        ...
        ## for the pairwise comparison between treatments t1 and t2 the treatment 1 is in the second position and the treatment 2 in the first position (i.e. t2>t1)
        ## here 'no_effect1' can be either the null value (if relax.roe=FALSE (default)) or it can be the upper bound of ROE if relax.roe=T.
        ## the relaxed criterion allows for more "wins" to be identified from the TTC as in this case the statistical significance of the effect does not alter the preference
