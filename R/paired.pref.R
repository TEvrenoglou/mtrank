#' Calculate pairwise probabilities using the output of the \code{\link{mtrank}} function.
#' 
#' @description
#' This function uses the estimates of the ability and tie prevalence parameters obtained by the \code{\link{mtrank}} function
#' and calculates pairwise probabilities about the preference or the tie between two treatments based on Equations 7 and 8 
#' from the manuscript by Evrenoglou et al.
#
#' @param x An object of class "mtrank.fit".
#' @param treat1 The first treatment participating in the treatment comparison.
#' @param treat2 The second treatment participating in the treatment comparison.
#' @param prob_type The probability of interest 
#'   
#' @details  
#' Pairwise probabilities between any two treatments in the network can be calculated using the ability estimates
#' obtained from the \code{\link{mtrank}} function and the Equations 7 and 8 from Evrenoglou et al. The probabilities
#' are calculated in the direction \code{treat1} vs \code{treat2}. The available probability types are
#' \itemize{
#' \item "better": Calculates the probability that \code{treat1} is better than \code{treat2}.
#' \item "equal": Calculates the probability that \code{treat1} is equal to \code{treat2}.
#' \item "worse": Calculates the probability that \code{treat1} is worse than \code{treat2}.
#' \item "worse": Calculates all the previous probabilities simultaneously.
#' }
#' 
#' Please note that all the arguments of this function are mandatory.
#' 
#' @return
#' The probability (or probabilities) of interest for the comparison \code{treat1} vs \code{treat2} based on the argument \code{prob_type}.
#' 
#' @references
#' \itemize{
#' \item Theodoros Evrenoglou, Adriani Nikolakopoulou, Guido Schwarzer, Gerta Rücker, Anna Chaimani (2024):
#' Producing treatment hierarchies in network meta-analysis using probabilistic models and treatment-choice criteria.
#' \emph{https://arxiv.org/abs/2406.10612}
#' }



paired_pref <- function(x,treat1,treat2,prob_type=c("better","equal","worse","all"),...){
  
  if(inherits(x,"mtrank.fit")){
    
    all_treats <- attributes(x)$all_treats
    
    preference.data <- attributes(x)$preference.data
    
    ref <- attributes(x)$ref
    
  } else{
    
    stop("Argument 'x' is not of 'mtrank' class.") 
    
  }
  
  if(treat1 %!in% all_treats){
    
    stop("Incorrect treatment name for argument 'treat1'. Please, use one of the available treatments: ",paste(all_treats,collapse = ", "))
    
  }
  
  if(treat2 %!in% all_treats){
    
    stop("Incorrect treatment name for argument 'treat2'. Please, use one of the available treatments: ",paste(all_treats,collapse = ", "))
    
  }
  
  if(missing(x)){
    
    stop("Please specify argument 'x'.")
    
  }
  
  if(missing(treat1)){
    
  stop("Please specify argument 'treat1'.")  
    
  }
  
  if(missing(treat2)){
    
    stop("Please specify argument 'treat2'")
    
  }
  
  if(missing(prob_type)){
    
    stop("Please specify the probability type of interest.")
    
  }
  
  ### to calculate the paired preferences we need access to the results of the mtrank() with ref=NULL so that we have 
  ### the ability estimates for each treatment  
  
  if(!is.null(ref)){
    
    model <- mtrank(data=preference.data,ref = NULL)     
    
  }else{
    
    model <- x
    
  }
  
  ### extract the ability estimate for 'treat1' and 'treat2' in a natural scale  
  
  psi.t1 <- exp(model$estimates[model$estimates$treatment==treat1,]$log_ability)
  
  psi.t2 <- exp(model$estimates[model$estimates$treatment==treat2,]$log_ability)
  
  ### extract 'v' parameter estimate
  
  v <- model$v
  
  ### get probability that 'treat1' is better than 'treat2' using Equation 7 from the main manuscript
  
  pr_t1.t2 <- (psi.t1)/(psi.t1+psi.t2+v*sqrt(psi.t1*psi.t2))
  
  ### get probability that 'treat1' is tied to 'treat2' using Equation 8 from the main manuscript
  
  pr_t1.t2.tie <- (v*sqrt(psi.t1*psi.t2))/(psi.t1+psi.t2+v*sqrt(psi.t1*psi.t2))
  
  ### get probability that 'treat1' is worse than 'treat2' using that Pr(treat1>treat2)+Pr(treat1=treat2)+Pr(treat1<treat2)=1
  
  pr_t2.t1 <- 1-pr_t1.t2-pr_t1.t2.tie
  
  ### report the results according to the prob_type of interest
  
  txt1 <- paste("The probability that ",treat1," is better than ",treat2," is equal to: ", round(pr_t1.t2,digits = 4)) 
  
  txt2 <- paste("The probability that ",treat1," is tied to ",treat2," is equal to: ", round(pr_t1.t2.tie,digits = 4))
  
  txt3 <- paste("The probability that ",treat1," is worse than ",treat2," is equal to: ", round(pr_t2.t1,digits = 4))  
  
  if(prob_type=="better"){
    
    txt <- txt1
    
  }else if(prob_type=="tie"){
    
    txt <- txt2 
    
  }else if(prob_type=="worse"){
    
    txt <- txt3 
    
  } else if(prob_type=="all"){
    
    txt <- list(txt1,txt2,txt3) 
    
  }
  
  res <- txt
  
  class(res) <- "mtrank"
  
  return(res)
  
  
}